...
app.get("/secrets", async (req, res) => {
    if (req.isAuthenticated()) {
        try {
            const getSecretQuery = "SELECT secret FROM users WHERE email = $1"
            const result = await db.query(getSecretQuery, [req.user.email]);
            console.log(result);
...
app.post("/register", async (req, res) => {
    ...
    try {
        ...
        const checkResult = await db.query(getUserQuery, [email]);
